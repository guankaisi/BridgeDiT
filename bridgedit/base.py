...
def prepare_args(now, args):
    ...
    if args.resume:
        ...
        if os.path.isfile(args.resume):
            ...
            idx = len(paths)-paths[::-1].index("log")+1
...
def prepare_config(args, unknown, nowname, log_dir):
    ...
    if trainer_config["devices"]:
        ...
        # assert trainer_config["strategy"] in ["ddp", "ddp_find_unused_parameters_false", "deepspeed"], "Only support ddp strategy for now"
...
class DataModuleFromConfig(pl.LightningDataModule):
    def __init__(self, batch_size, persistent_workers=False, prefetch_factor=2, num_workers=None, 
    ...
    def _train_dataloader(self):
        if self.dataset_names["train"] in ["vggas", "hits", "favd", "tavg", "avsync15", "vggsound", "vggss", "greatesthits", "landscape"]:
            return DataLoader(self.datasets["train"], batch_size=self.batch_size, shuffle=True, collate_fn=None,
    ...
    def _val_dataloader(self):
        if self.dataset_names["validation"] in ["vggas", "favd", "tavg", "avsync15", "vggsound", "vggss", "greatesthits", "landscape"]:
    ...
    def _test_dataloader(self):
        if self.dataset_names["test"] in ["vggas", "favd", "tavg", "avsync15", "vggsound", "vggss", "greatesthits", "landscape"]:
            return DataLoader(self.datasets["test"], batch_size=self.batch_size, shuffle=False, collate_fn=None,
                              persistent_workers=self.persistent_workers, num_workers=self.num_workers,
                              prefetch_factor=self.prefetch_factor,)
        else:
