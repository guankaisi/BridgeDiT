...
class SynchformerAudioDataset(Dataset):
    ...
    def __init__(self, datalist: List[Path], duration: float = 8.0):
        ...
        # self.expected_length = int(44100 * duration)
        self.resampler = {}
    ...
    def sample(self, idx: int):
        ...
        self.expected_length = int(sr * self.duration)
        if sr >= 44100:
            if sr not in self.resampler:
                self.resampler[sr] = torchaudio.transforms.Resample(sr, 44100)
            waveform = self.resampler[sr](waveform)
        if waveform.shape[0] <= self.expected_length:
            repeat_times = int(self.expected_length // waveform.shape[0]) + 1  # 确保是整数
            waveform = waveform.repeat(repeat_times)  # 沿着dim=0重复
