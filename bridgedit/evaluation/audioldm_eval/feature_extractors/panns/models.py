...
class Cnn14(nn.Module):
    def __init__(
        ...
        _ckpt_dir = os.path.join(os.environ.get("T2SV_MODEL_ROOT", os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", "..", "..", "models"))), "ckpt")
        if not os.path.exists(os.path.join(_ckpt_dir, "Cnn14_mAP=0.431.pth")):
            print("Download pretrained checkpoints of Cnn14.")
            os.makedirs(_ckpt_dir, exist_ok=True)
            os.system("wget -P %s %s" % (_ckpt_dir, "https://zenodo.org/record/3576403/files/Cnn14_mAP%3D0.431.pth"))
            os.system("wget -P %s %s" % (_ckpt_dir, "https://zenodo.org/record/3987831/files/Cnn14_16k_mAP%3D0.438.pth"))

        # self.init_weight()
        if sample_rate == 16000:
            state_dict = torch.load(os.path.join(_ckpt_dir, "Cnn14_16k_mAP=0.438.pth"), weights_only=False)
            self.load_state_dict(state_dict["model"])
        elif sample_rate == 32000:
            state_dict = torch.load(os.path.join(_ckpt_dir, "Cnn14_mAP=0.431.pth"))
            self.load_state_dict(state_dict["model"])
