...
def generate_video_raw_caption(video_js, batch_size=1, mllm=None, processor=None, stats=None):
    ...
    if stats is not None:
        stats["video_raw_caption"] = {"seconds": elapsed, "n_samples": n_done}
    return video_js
...
def generate_audio_raw_caption(video_js, batch_size=1, llm=None, stats=None):
...
def run_semantic_checker_train(video_js, batch_size=1, llm=None, tokenizer=None, stats=None):
    ...
    if stats is not None:
        stats["semantic_checker"] = {"seconds": elapsed, "n_samples": n_done}
    return video_js


def run_semantic_checker_infer(user_prompts, batch_size=1, llm=None, tokenizer=None, stats=None):
...
def run_cross_modal_rewriter(samples, batch_size=1, llm=None, tokenizer=None, stats=None):
    ...
    if stats is not None:
        stats["cross_modal_rewriter"] = {"seconds": elapsed, "n_samples": n_done}
    return samples
...
def run_crr_training_pipeline(
    ...
    Returns:
        (video_js, stats) where stats records per-stage wall-clock seconds and
        sample counts (model load/unload time NOT included).
    """
    stats = {}
    ...
    video_js = generate_video_raw_caption(
        video_js, batch_size=batch_size, mllm=mllm, processor=processor, stats=stats
    )
    del mllm, processor
    ...
    video_js = generate_audio_raw_caption(
        video_js, batch_size=batch_size, llm=audio_llm, stats=stats
    )
    ...
    video_js = run_semantic_checker_train(
        video_js, batch_size=batch_size, llm=llm, tokenizer=tokenizer, stats=stats
    )
...
def run_crr_inference_pipeline(
    ...
    user_prompts = run_semantic_checker_infer(
        user_prompts, batch_size=batch_size, llm=llm, tokenizer=tokenizer, stats=stats
    )
...
def format_timing_summary(stats, stage_order=None):
    """Pretty-print a per-stage timing summary.

    stats: {stage_name: {"seconds": float, "n_samples": int}}
    Returns the formatted string (also safe to print directly).
    """
    if not stats:
        return "[CRR] No timing stats recorded."

    if stage_order is None:
        stage_order = list(stats.keys())

    total_seconds = sum(stats[s]["seconds"] for s in stage_order if s in stats)
    # Per-video total = sum of per-stage averages. Robust to stages dropping
    # samples on error (e.g. an audio file failed to load).
    per_video_total = 0.0
    for s in stage_order:
        if s in stats and stats[s]["n_samples"] > 0:
            per_video_total += stats[s]["seconds"] / stats[s]["n_samples"]

    bar = "=" * 78
    sep = "-" * 78
    lines = [
        bar,
        "  CRR Pipeline Timing Summary  (model load/unload NOT included)",
        bar,
        f"  {'Stage':<28} {'Total (s)':>14} {'Per-video (s)':>16} {'N':>10}",
        sep,
    ]
    for s in stage_order:
        if s not in stats:
        ...
        sec = stats[s]["seconds"]
        n = stats[s]["n_samples"]
        per = sec / n if n > 0 else 0.0
        lines.append(f"  {s:<28} {sec:>14.3f} {per:>16.3f} {n:>10d}")
    lines.append(sep)
    lines.append(
        f"  {'TOTAL':<28} {total_seconds:>14.3f} {per_video_total:>16.3f}"
    )
    lines.append(bar)
    return "\n".join(lines)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--limit", type=int, default=1,
                        help="Number of videos / prompts to process. Default 1 "
                             "for a fast single-video timing benchmark. "
    ...
    parser.add_argument("--timing_file", type=str, default=None,
                        help="Optional path to dump the timing summary JSON. "
                             "Defaults to <output_file>.timing.json")
    args = parser.parse_args()

    args.audio_llm_path = args.audio_llm_path or model_path("Qwen2-Audio-7B-Instruct")
    args.mllm_path = args.mllm_path or model_path("Qwen2.5-VL-7B-Instruct")
    args.llm_path = args.llm_path or model_path("Qwen2.5-7B-Instruct")

    out_dir = os.path.dirname(args.output_file)
    if out_dir:
        os.makedirs(out_dir, exist_ok=True)

    timing_file = args.timing_file or (args.output_file + ".timing.json")
    ...
    if args.mode == "train":
        ...
        # Apply limit (default 1) for a quick single-video timing benchmark.
        if args.limit and args.limit > 0:
            keys = list(video_js.keys())[:args.limit]
            video_js = {k: video_js[k] for k in keys}
            print(f"[CRR] Limited to {len(video_js)} video(s) for timing benchmark.")

        video_js, stats = run_crr_training_pipeline(
            video_js=video_js,
        ...
        summary = format_timing_summary(
            ...
            stage_order=[
                ...
                "audio_raw_caption",
                "semantic_checker",
                "cross_modal_rewriter",
            ],
        )
        print("\n" + summary)
        with open(timing_file, 'w') as f:
            json.dump(stats, f, indent=2, ensure_ascii=False)
        print(f"[CRR] Timing stats saved to {timing_file}")
        ...
        if args.limit and args.limit > 0:
            keys = list(user_prompts.keys())[:args.limit]
            user_prompts = {k: user_prompts[k] for k in keys}
            print(f"[CRR] Limited to {len(user_prompts)} prompt(s) for timing benchmark.")

        user_prompts, stats = run_crr_inference_pipeline(
        ...
        summary = format_timing_summary(
            stats,
            stage_order=["semantic_checker", "cross_modal_rewriter"],
        )
        print("\n" + summary)
        with open(timing_file, 'w') as f:
            json.dump(stats, f, indent=2, ensure_ascii=False)
